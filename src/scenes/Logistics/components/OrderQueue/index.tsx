...
const OrderQueue = (): ReactElement => {
  ...
  const getHoldup = (): string | null => {
    ...
    for (const content of contents) {
      ...
      if (!content.location) errs.push("Each order requires a destination.");
      if (!content.date) errs.push("Each order requires a delivery date.");
            ...
            header: (
              <Box sx={{ paddingBottom: 2 }}>
                ...
                <Typography color="textSecondary" variant="body2">
                  Each line in the queue will produce a linked Order, Itinerary,
                  and BOL.
                </Typography>
              </Box>
            ),
            ...
            footer: (
              <Box sx={{ display: "flex" }}>
                ...
                <Box sx={{ display: "flex", gap: 2 }}>
                  ...
                  <Tooltip arrow placement="top" title={holdup || ""}>
                    <Box>
                      <Fab
                        disabled={
                          Boolean(holdup) || processing || contents.length == 0
                        }
                        variant="extended"
                        sx={{
