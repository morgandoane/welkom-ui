...
const OrderForm = (): ReactElement => {
  ...
  const locations = locationData.data ? locationData.data.locations.items : [];
  ...
  const popContent = (index?: number) => {
    ...
    if (copy.contents !== undefined) {
      if (index == undefined) {
        ...
        setFocusedContent({ ...newContent, index: copy.contents.length });
  ...
  const getHoldup = (): string | null => {
    ...
    if (edits.contents)
      for (const { item, unit, quantity, location } of edits.contents) {
        ...
        if (!location) contentErrors.push("Each content needs a location");
        if (quantity == 0) contentErrors.push("Each content needs a quantity");
      }
          ...
          content:
            result && result.success == true ? (
                ...
                onComplete={() => {
                  if (result && result.success == true) {
                    if ("updateOrder" in result.data)
                      nav("/logistics/orders/" + result.data.updateOrder._id);
                    else
                      nav("/logistics/orders/" + result.data.createOrder._id);
                  }
                }}
              />
              ...
              <Box sx={{ paddingTop: 2 }}>
                ...
                <Box sx={{ maxWidth: 600 }}>
                  <FormRow>
                    <CompanyField
                      ...
                      onChange={(val) => {
                        setEdits({ ...edits, vendor: val || undefined });
                      }}
                    />
                    <Box
                      sx={{
                    ...
                    <CompanyField
                      ...
                      onChange={(val) => {
                        setEdits({ ...edits, customer: val || undefined });
                      }}
                    />
                  </FormRow>
                </Box>
                    ...
                    edits.contents.map((content, index) => {
                      ...
                      return (
                        <ButtonBase
                          ...
                          onClick={() =>
                            setFocusedContent({ ...content, index })
                          }
                        >
                          {isReady ? (
                ...
                <Box p={0.5} />
                <Button
                  disabled={!focusedCustomer || !focusedVendor}
                  startIcon={<MdAdd />}
                  variant="text"
                  onClick={() => popContent()}
                >
              ...
              <Tooltip title={holdup || ""} arrow>
                <Box>
                  <LoadingButton
                    ...
                    endIcon={<MdCheck />}
                  >
                    {id !== undefined && id.length > 0 ? "Save" : "Submit"}{" "}
                    order
                  </LoadingButton>
                </Box>
              </Tooltip>
      ...
      >
        ...
        <FormRow>
          <LocationField
            ...
            company={focusedCustomer ? focusedCustomer._id : undefined}
            onChange={(val) => {
              if (focusedContent)
                setFocusedContent({ ...focusedContent, location: val || "" });
            }}
          ...
          <DateField
            ...
            onChange={(val) => {
              if (focusedContent && val)
                setFocusedContent({ ...focusedContent, due: val });
            }}
          />
        </FormRow>
        <FormRow>
          <ItemField
            onChange={(val) => {
              if (focusedContent)
                setFocusedContent({ ...focusedContent, item: val || "" });
            }}
            ...
            <NumberField
              ...
              onChange={(val) => {
                if (focusedContent && val !== null)
                  setFocusedContent({ ...focusedContent, quantity: val });
              }}
              value={focusedContent ? focusedContent.quantity : null}
            />
          </Box>
          <UnitField
            onChange={(val) => {
              if (focusedContent)
                setFocusedContent({ ...focusedContent, unit: val || "" });
            }}
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          ...
          <Box>
            <CarefulButton
              onClick={() => {
                popContent(focusedContent ? focusedContent.index : -1);
