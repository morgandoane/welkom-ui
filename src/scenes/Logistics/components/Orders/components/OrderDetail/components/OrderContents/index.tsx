  ...
  return (
    <Box sx={{ paddingTop: 3 }}>
      {order.contents.map((content) => {
        ...
        const location = locations.find((i) => i._id == content.location._id);
        const isReady =
          unit !== undefined && item !== undefined && location !== undefined;
        ...
        if (!isReady)
          ...
          return (
            <Box sx={{ marginBottom: 4 }}>
              ...
              >
                {`${content.quantity} ${
                  unit[content.quantity == 1 ? "english" : "english_plural"]
                }`}
                <MdChevronRight />
