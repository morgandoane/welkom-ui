...
const WarehouseCalendar = (props: WarehouseCalendarProps): ReactElement => {
  ...
  React.useEffect(() => {
    if (location)
      if (
        ...
        setFilter((f) => ({
          ...f,
          [view == "receiving" ? "to_location" : "from_location"]: location,
        }));
      }
  ...
  const getBolGroups = (): EventGroupProps<Bol>[] => {
    ...
    for (const bol of bols) {
      const dateVal = new Date(bol[view == "shipping" ? "from" : "to"].date);
    ...
    for (const dateKey of Object.keys(groupedByDate)) {
      ...
      for (const itinerary_id of Object.keys(groupedByItinerary)) {
        const itinerary = itineraries.find((i) => i._id === itinerary_id);
    ...
    <React.Fragment>
      <Calendar
        ...
        getEventProps={(bol, index) => ({
          ...
          primary: bol.contents.map((c) => `${c.item.english}`).join(", "),
          secondary: bol.contents
            .map(
              (c) =>
                `${c.quantity} ${
                  c.unit[c.quantity == 1 ? "english" : "english_plural"]
                }`
            )
            .join(", "),
          onClick: (bol, event) => {
