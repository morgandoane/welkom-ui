...
const WarehouseTable = (props: WarehouseTableProps): ReactElement => {
  ...
  const [filterEdits, setFilterEdits] = React.useState<BolFilter | null>(null);
        ...
        {{
            ...
            <Box sx={{ display: "flex", gap: 2, paddingBottom: 2 }}>
              <Box>
                <SearchField
                  label="Search by BOL"
                  value={filter.code || ""}
                  onChange={(val) => setFilter({ ...filter, code: val || "" })}
                />
              </Box>
              <Button
          ...
          content: (
            <DataGrid
              ...
              onPageChange={(page) => setFilter({ ...filter, skip: page * 25 })}
              ...
              columns={[
                { field: "code", headerName: "BOL #", width: 120 },
                {
                  flex: 1,
                  field: "orders",
                  headerName: "Order #",
                ...
                { field: "status", headerName: "BOL Status", width: 120 },
                {
                  flex: 1,
                  field: "from",
                  headerName: "From",
                ...
                },
                {
                  ...
                  valueGetter: (data) => {
                    ...
                    return `${to.company.name}${
                      to.location
                        ? ` (${
                            to.location.label || to.location.address?.city || ""
                          })`
                        : ""
                    }`;
                  },
                ...
                {
                  flex: 1,
                  field: "receiving" ? "receipts" : "shipments",
                  headerName: view == "receiving" ? "Receipts" : "Shipments",
                  valueGetter: (data) => {
                    const fulfillments = (data.row as Bol)[
                      view == "receiving" ? "receipts" : "shipments"
                    ];
                    if (fulfillments.length == 0) return "None";
                    return fulfillments
                      .map((f) =>
                        f.lots.map(
                          (l) =>
                            `${l.item.english} (${format(
          ...
          {{
            ["From company"]: [
              <CompanyField
                naked
                key="from_company"
                value={filterEdits ? filterEdits.from_company || "" : ""}
                onChange={(val) => {
                  if (filterEdits)
                    setFilterEdits({ ...filterEdits, from_company: val || "" });
                }}
              />,
              filterEdits !== null && filterEdits.from_company !== undefined,
              (checked) => {
                if (filterEdits) {
                  setFilterEdits({
                    ...filterEdits,
            ...
            ["To company"]: [
              <CompanyField
                naked
                key="to_company"
                value={filterEdits ? filterEdits.to_company || "" : ""}
                onChange={(val) => {
                  if (filterEdits)
                    setFilterEdits({ ...filterEdits, to_company: val || "" });
                }}
              />,
              filterEdits !== null && filterEdits.to_company !== undefined,
              (checked) => {
                if (filterEdits) {
                  setFilterEdits({
                    ...filterEdits,
            ...
            ["BOL number"]: [
              <ItemField
                naked
                key="itemFilter"
                value={filterEdits ? filterEdits.code || "" : ""}
                onChange={(val) => {
                  if (filterEdits)
                    setFilterEdits({ ...filterEdits, code: val || "" });
                }}
              />,
              filterEdits !== null && filterEdits.code !== undefined,
              (checked) => {
                if (filterEdits) {
                  setFilterEdits({
                    ...filterEdits,
