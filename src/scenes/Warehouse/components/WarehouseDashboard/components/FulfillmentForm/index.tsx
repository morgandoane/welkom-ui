...
const FulfillmentForm = (props: {
  ...
  >({
    ...
    bol: bol_id || "",
    type:
      action == "receive" ? FulfillmentType.Receipt : FulfillmentType.Shipment,
    items: [],
    location: "",
    company: "",
  });
    ...
    onCompleted: ({ fulfillment: { lots, type, location, company } }) => {
      setState({
        ...
        items: lots.map(({ item, contents }) => ({
          item: item._id,
          lots: contents.map(
            ({ lot: { code, company, location }, quantity, unit }) => ({
              code,
              quantity,
              unit: unit._id,
              ...
              <Box>
                <Box sx={{ display: "flex", flexFlow: "row" }}>
                  <Box>
                    ...
                    <Typography variant="h6">{bol ? bol.code : ""}</Typography>
                    <Typography variant="h6">
                      {bol
                        ? bol.contents.map((c) => c.item.english).join(", ")
                        : ""}
                    </Typography>
                  </Box>
                  <Box sx={{ flex: 1 }} />
                ...
                <Stepper activeStep={index} orientation="vertical">
                      ...
                      <Box sx={{ maxWidth: 400, padding: 2 }}>
                        {bol && (
                          <FormRow gap={4}>
                            <TextFormField
                              label="BOL number"
                              value={state.bol_code_override || bol.code || ""}
                              onChange={(val) => {
                                setState({
                                  ...state,
                                  bol_code_override: val || undefined,
                                });
                              }}
                            />
                          </FormRow>
                        ...
                        <Box sx={{ display: "flex", gap: 1 }}>
                          <Button
                            disabled={
                              !bol || (!bol.code && !state.bol_code_override)
                            }
                      ...
                      if (content)
                        return (
                          <Step key={"content_" + itemIndex}>
                            ...
                            >
                              <FulfillmentItemForm
                                initiate={state._type !== "update"}
                                droppable={lots.length > 1}
                                tiny_item={content.item}
                                item={item}
                                setItem={(value) => {
                                  const copy = { ...state };
                                  copy.items[itemIndex] = value;
                                  setState(copy);
                                }}
                                ...
                                drop={(i) => {
                                  const copy = { ...state };
                                  copy.items[itemIndex].lots.splice(i, 1);
                                  setState(copy);
                                }}
                              />
                            </StepContent>
                  ...
                  <Step>
                    ...
                    <StepContent>
                      <Box sx={{ p: 2 }}>
                        <FulfillmentReview value={state} bol={bol} />
