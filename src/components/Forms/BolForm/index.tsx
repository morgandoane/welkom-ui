...
const BolForm = (): ReactElement => {
  ...
  const getFocusedHoldup = (): string | null => {
    if (focusedContent) {
      if (state.contents.map((c) => c.item).includes(focusedContent.item)) {
  ...
  const getBolHoldup = (): string | null => {
    ...
    if (state.to.company == "") return "Please select a company to dropoff at";
          ...
          {{
            header: (
              <Box sx={{ paddingBottom: 2 }}>
                ...
                <PageTitle>
                  {id && id.length > 0 ? "Update BOL" : "Create BOL"}
                </PageTitle>
              ...
              <Box>
                ...
                >
                  ...
                  >
                    ...
                    <DateField
                      ...
                      onChange={(date) => {
                        if (date) {
                          setState({ ...state, from: { ...state.from, date } });
                        }
                      }}
                    />
                    <Box sx={{ display: "flex", flexFlow: "row" }}>
                      <BooleanField
                        label="Location"
                        value={from.location !== undefined}
                        onChange={(val) => {
                          setState({
                            ...state,
                            from: {
                              ...state.from,
                              location: val == true ? "" : undefined,
                            },
                          });
                        }}
                      />
                      <Box sx={{ flex: 1 }}>
                        <Collapse in={from.location !== undefined}>
                          <LocationField
                  ...
                  <Box sx={{ display: "flex", fontSize: "1.75rem" }}>
                    <MdChevronRight />
                  </Box>
                  <Box sx={{ flex: 1 }}>
                    <Box
                    ...
                    >
                      <Typography variant="h6">Dropoff</Typography>
                      <CompanyField
                        label="To company"
                        value={to.company}
                        onChange={(val) =>
                      ...
                      <DateField
                        ...
                        onChange={(date) => {
                          if (date) {
                            setState({ ...state, to: { ...state.to, date } });
                          }
                        }}
                      />
                      <Box sx={{ display: "flex", flexFlow: "row" }}>
                        ...
                        />
                        <Box sx={{ flex: 1 }}>
                          <Collapse in={to.location !== undefined}>
                            <LocationField
                              company={to.company}
                              label="To location"
                              value={to.location || ""}
                ...
                <Box sx={{ paddingTop: 4 }}>
                  ...
                  {contents.map((content, contentIndex) => {
                    ...
                    const isReady = unit !== undefined && item !== undefined;
            ...
            footer: (
              ...
              >
                <Box sx={{ padding: 2 }}>
                  <Tooltip title={bolHoldup || ""} arrow placement="top">
                    <Box>
                      <LoadingButton
                        onClick={submit}
                        variant="contained"
      ...
      >
        ...
        <FormRow>
          <NumberField
            ...
            onChange={(val) => {
              if (focusedContent)
                setFocusedContent({ ...focusedContent, quantity: val || 0 });
            }}
          />
          <UnitField
              ...
              <CarefulButton
                onClick={() => {
                  if (focusedContent) {
                    ...
                    contents.splice(focusedContent.index, 1);
              ...
              <Button
                ...
                onClick={() => {
                  if (focusedContent) {
                    ...
                    setState({ ...state, contents: [...state.contents, rest] });
                    setFocusedContent(null);
                  }
                }}
              >
